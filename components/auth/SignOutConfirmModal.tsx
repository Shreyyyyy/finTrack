...
export function SignOutConfirmModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-150">
      <div className="w-full max-w-sm bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-2xl overflow-hidden p-6 space-y-5 animate-in zoom-in-95 duration-150">
        {/* Icon & Close */}
        ...
        <div className="space-y-1.5 text-left">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
            {userName ? (
              <>
                You are currently signed in as <strong className="text-slate-700 dark:text-slate-200">{userName}</strong>.
              </>
            ) : null}{' '}
        ...
        <div className="grid grid-cols-2 gap-3 pt-1">
          <button
            ...
            onClick={onClose}
            disabled={isSigningOut}
            className="w-full py-2.5 rounded-xl border border-slate-200 dark:border-slate-800 text-xs font-semibold text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors disabled:opacity-50"
          >
            Cancel
          </button>
          <button
