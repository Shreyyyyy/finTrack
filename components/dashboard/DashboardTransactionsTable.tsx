...
export function DashboardTransactionsTable({
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-3xl p-4 sm:p-6 border border-slate-200/80 dark:border-slate-800 shadow-sm space-y-5">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
            Total Spend in View: <span className="font-bold text-slate-900 dark:text-white">{formatINR(totalFilteredAmount)}</span>
          </p>
        </div>
        ...
        <Link
          href="/add"
          className="inline-flex items-center justify-center gap-1.5 px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold shadow-sm active:scale-95 transition-all self-start md:self-auto"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-12 gap-3">
        ...
        <div className="sm:col-span-6 relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200/80 dark:border-slate-700/80 text-xs text-slate-900 dark:text-white placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500/30"
          />
        </div>
        ...
        <div className="sm:col-span-3">
          <select
            ...
            className="w-full px-3 py-2 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200/80 dark:border-slate-700/80 text-xs font-semibold text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-emerald-500/30"
          >
      ...
      {filteredExpenses.length === 0 ? (
        <div className="text-center py-12 space-y-2 bg-slate-50/50 dark:bg-slate-800/20 rounded-2xl border border-dashed border-slate-200 dark:border-slate-800">
        ...
        <>
          ...
          <div className="block md:hidden space-y-2.5">
            {visibleExpenses.map((exp) => (
              <div
                key={exp.id}
                className="p-3.5 rounded-2xl bg-slate-50/80 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 flex items-center justify-between gap-3 shadow-2xs"
              >
          ...
          <div className="hidden md:block overflow-x-auto rounded-2xl border border-slate-200/80 dark:border-slate-800">
            <table className="w-full text-left border-collapse text-xs">
              <thead>
                <tr className="bg-slate-100/70 dark:bg-slate-800/60 text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wider text-[10px] border-b border-slate-200 dark:border-slate-800">
                ...
                {visibleExpenses.map((exp) => (
                  <tr key={exp.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                    ...
                    <td className="py-3 px-4">
                      ...
                      {exp.note && (
                        <div className="text-[11px] text-slate-400 truncate max-w-xs">{exp.note}</div>
                      )}
                    </td>
                    <td className="py-3 px-4 whitespace-nowrap">
                      <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-medium">
                    ...
                    <td className="py-3 px-4 text-right font-black text-slate-900 dark:text-white whitespace-nowrap">
                      {formatINR(exp.amount)}
                    </td>
          ...
          {filteredExpenses.length > visibleCount && (
            <div className="pt-2 text-center">
              <button
                ...
                className="px-4 py-2 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-xs font-bold text-slate-700 dark:text-slate-300 transition-colors shadow-2xs"
              >
