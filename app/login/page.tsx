...
import { DB_ADMIN_USERNAME, isDbAdminIdentifier } from '@/lib/auth/adminConfig';
...
export default function LoginPage() {
  ...
  // Parse any OAuth error returned via query parameter
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      if (errorParam) {
        if (errorParam === 'oauth_failed') {
          setErrorMessage(
            'Google authentication could not be completed. Please ensure the Google provider is enabled in your Supabase Dashboard.'
          );
        } else {
  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    const authIdentifier = isAdminMode ? (email.trim() || DB_ADMIN_USERNAME) : email.trim();

    if (!authIdentifier) {
      setErrorMessage(isAdminMode ? 'Please enter username.' : 'Please enter your email or username.');
      return;
    }
    if (!password) {
    ...
    try {
      const searchParams =
        typeof window !== 'undefined'
          ...
          : null;
      const isLoggingInAsAdmin = isDbAdminIdentifier(authIdentifier) || isAdminMode;
      const defaultTarget = isLoggingInAsAdmin ? '/admin' : '/dashboard';
      const targetUrl = searchParams?.get('next') || defaultTarget;

      if (isSignUp) {
        const res = await signUpWithEmail(authIdentifier, password, displayName);
        ...
        const res = await signInWithEmail(authIdentifier, password);
        if (!res.success && res.error) {
          setErrorMessage(res.error);
        } else {
          window.location.href = targetUrl;
      ...
      <div className="w-full max-w-md bg-white dark:bg-slate-900 rounded-3xl p-6 sm:p-9 border border-sky-100 dark:border-slate-800 shadow-2xl space-y-6">
        ...
        {user || profile ? (
          ...
          <div className="space-y-5">
            ...
            {/* 1. Continue with Google (Member mode only) */}
            {!isAdminMode && (
              <>
                <button
                  type="button"
                ...
                {!isConfigured && (
                  <p className="text-[11px] text-amber-700 dark:text-amber-400 text-center font-medium bg-amber-50 dark:bg-amber-950/40 py-1.5 px-2.5 rounded-xl border border-amber-200/80 dark:border-amber-900/60">
                    💡 Supabase is running in offline demo mode. Set <code className="font-mono text-[10px] font-bold">.env.local</code> to enable Google OAuth.
                  </p>
                )}
